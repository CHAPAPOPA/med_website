...
from main.models import Speciality, Doctor, Appointment, Diagnostic
...
class AppointmentListViewTest(TestCase):
    def setUp(self):
        self.client = Client()
        self.user = User.objects.create_user(email='user@test.com', password='pass', birthday="2000-01-01")
        ...
        diagnostic = Diagnostic.objects.create(title="ЭКГ", price=1000, speciality=speciality)
        diagnostic.doctor.add(doctor)
        Appointment.objects.create(user=self.user, diagnostic=diagnostic, doctor=doctor,
